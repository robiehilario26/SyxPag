 ...
 * @author User
 */
@WebServlet(name = "get_pricing_arraylist", urlPatterns = {"/get_pricing_arraylist"})
public class get_pricing_arraylist extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (get_action.equalsIgnoreCase("add")) {
            ...
            String emp_level = linedao.search_if_existed(get_pk);
...
//
//                }
                    linedao.insertPricing_table(item);
                }
            } else {
            ...
            linedao.delete_pricing_by_customer(get_pk);
        } else {
            for (int i = 0; i < f_arrays.length; i++) {
//                DAO_product_mainte linedao = new DAO_product_mainte();
                SalesOrderModel item = new SalesOrderModel();
